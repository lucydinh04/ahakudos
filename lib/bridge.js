...
// Methods the browser may call through /api/bridge (layAnhNen is only used by /api/background).
export const BROWSER_METHODS = Object.freeze(Object.keys(METHODS).filter(m => m !== 'layAnhNen'));
export const UPLOAD_METHODS = Object.freeze(['taiAnhNen']);
...
export async function callGoogle(config, identity, method, args, fetchImpl = fetch) {
  ...
  try {
    ...
    if (!result.ok) {
      ...
      const status = code === 'BRIDGE_REJECTED' ? 502 : code === 'FORBIDDEN' ? 403 : code === 'NOT_IN_MASTER_DATA' ? 403 : code === 'UNAUTHENTICATED' ? 401 : code === 'KUDOS_NOT_AVAILABLE' ? 404 : code === 'BUSY' ? 503 : 400;
